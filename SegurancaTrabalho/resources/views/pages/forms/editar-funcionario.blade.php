...
<div class="row">
  <div class="col-lg-12">
    ...
    <form method="POST" action="{{ route('funcionarios.update', $funcionario->id) }}" novalidate>
      @csrf
      @method('PUT')
        ...
        <div class="card-body">
          <div class="row g-3">
            ...
            <div class="col-md-3">
              ...
              <input type="text" id="cpf" name="cpf" class="form-control" value="{{ old('cpf', $funcionario->cpf) }}" placeholder="000.000.000-00" required>
            </div>
            <div class="col-md-3">
              <label for="rg" class="form-label"><i data-lucide="id-card" class="me-1"></i>RG</label>
              <input type="text" id="rg" name="rg" class="form-control" value="{{ old('rg', $funcionario->rg) }}">
            </div>

            <div class="col-md-3">
              <label for="data_nascimento" class="form-label"><i data-lucide="calendar" class="me-1"></i>Nascimento</label>
              <input type="text" id="data_nascimento" name="data_nascimento" class="form-control js-date" value="{{ old('data_nascimento', $funcionario->data_nascimento ? \Carbon\Carbon::parse($funcionario->data_nascimento)->format('d/m/Y') : '') }}" placeholder="dd/mm/aaaa">
            </div>
            <div class="col-md-3">
              <label class="form-label d-block"><i data-lucide="users" class="me-1"></i>Gênero</label>
              <div class="d-flex gap-3">
                <div class="form-check">
                  <input class="form-check-input" type="radio" name="genero" id="generoM" value="M" @checked(old('genero', $funcionario->genero)=='M')>
                  <label class="form-check-label" for="generoM">Masculino</label>
                </div>
                <div class="form-check">
                  <input class="form-check-input" type="radio" name="genero" id="generoF" value="F" @checked(old('genero', $funcionario->genero)=='F')>
                  <label class="form-check-label" for="generoF">Feminino</label>
                </div>
                <div class="form-check">
                  <input class="form-check-input" type="radio" name="genero" id="generoO" value="O" @checked(old('genero', $funcionario->genero)=='O')>
            ...
            <div class="col-md-3">
              ...
              <select id="estado_civil" name="estado_civil" class="form-select">
                ...
                @foreach(['solteiro(a)','casado(a)','divorciado(a)','viúvo(a)'] as $ec)
                  <option value="{{ $ec }}" @selected(old('estado_civil', $funcionario->estado_civil)===$ec)>{{ $ec }}</option>
                @endforeach
              </select>
            </div>
            <div class="col-md-3">
              <label for="telefone" class="form-label"><i data-lucide="phone" class="me-1"></i>Telefone</label>
              <input type="text" id="telefone" name="telefone" class="form-control" value="{{ old('telefone', $funcionario->telefones->first()->numero ?? '') }}" placeholder="(00) 00000-0000">
            </div>
            <div class="col-md-6">
              <label for="email" class="form-label"><i data-lucide="mail" class="me-1"></i>E-mail</label>
              <input type="email" id="email" name="email" class="form-control" value="{{ old('email', $funcionario->email) }}" placeholder="email@dominio.com.br">
            </div>
          </div>
        </div>
      </div>
        ...
        <div class="card-body">
          <div class="row g-3">
            <div class="col-md-6">
              ...
              <select id="empresa_id" name="empresa_id" class="form-select js-select2" data-width="100%" data-placeholder="Selecione a empresa" required>
                ...
                @foreach($empresas as $empresa)
                  <option value="{{ $empresa->id }}" @selected(old('empresa_id', (string)$funcionario->empresa_id) == (string)$empresa->id)>{{ $empresa->razao_social }}</option>
                @endforeach
              </select>
            </div>
            ...
            <div class="col-md-6">
              ...
              <select id="cargo_id" name="cargo_id" class="form-select js-select2" data-width="100%" data-placeholder="Selecione o cargo">
                ...
                @foreach($cargos as $cargo)
                  <option value="{{ $cargo->id }}" data-empresa="{{ $cargo->empresa_id }}" @selected(old('cargo_id', (string)$funcionario->cargo_id) == (string)$cargo->id)>
            ...
            <div class="col-md-3">
              ...
              <input type="text" id="data_admissao" name="data_admissao" class="form-control js-date" value="{{ old('data_admissao', $funcionario->data_admissao ? \Carbon\Carbon::parse($funcionario->data_admissao)->format('d/m/Y') : '') }}" placeholder="dd/mm/aaaa">
            </div>
            <div class="col-md-5">
              <label for="setor" class="form-label"><i data-lucide="map-pin" class="me-1"></i>Setor / Lotação</label>
              <input type="text" id="setor" name="setor" class="form-control" value="{{ old('setor', $funcionario->setor) }}">
            </div>
            <div class="col-md-4">
              ...
              <select id="turno" name="turno" class="form-select">
                ...
                @foreach(['Diurno','Noturno','Misto','Revezamento'] as $t)
                  <option value="{{ $t }}" @selected(old('turno', $funcionario->turno)===$t)>{{ $t }}</option>
                @endforeach
              </select>
            </div>
          </div>
    ...
    (function($) {
      ...
      function filtrarCargos() {
        ...
        $('#cargo_id option').each(function(){
          ...
          if (!$(this).val()) return;
          const show = !empId || !optEmp || String(optEmp) === String(empId);
          $(this).toggle(show);
        });
      }
      ...
      filtrarCargos();
      var selectedCargo = '{{ (string)$funcionario->cargo_id }}';
      if (selectedCargo) {
        $('#cargo_id').val(selectedCargo).trigger('change');
      }
    })(jQuery);
  </script>
@endpush
